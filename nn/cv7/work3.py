...
#input layer
my_model.add(keras.layers.Dense(100,input_dim=784))
my_model.add(keras.layers.Activation('relu'))
my_model.add(keras.layers.Dropout(0.2))

#hidden layer1
my_model.add(keras.layers.Dense(200))
my_model.add(keras.layers.Activation('relu'))
my_model.add(keras.layers.Dropout(0.2))

#hidden layer2
my_model.add(keras.layers.Dense(300))
my_model.add(keras.layers.Activation('relu'))
my_model.add(keras.layers.Dropout(0.2))
...
my_model.fit(x = train_x, y =  train_y, batch_size=batch_size,epochs=epoch_num, validation_data=(test_x,test_y),
             callbacks=[TensorBoard(log_dir='log/mnist2')])

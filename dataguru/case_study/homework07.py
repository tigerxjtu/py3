...
#排序
bh2 = pd.read_csv("D:/data/bh2.csv",header=None)
bh2=bh2.ix[:50]
N = bh2.shape[0]
n = bh2.shape[1]
        ...
        while j<n:
            if (mark[i]==0).all():
                ...
                if(R==np.nan): 
                    ...
                    mark[i][0,j]=1
                    for c in range(N)[1:]:
                        if bh2.iloc[c,j]!=bh2.iloc[c-1,j]:
                            mark[i][c,j]=1
#                        else:
#                            mark[i][c,j]=0
                    j=j+1
